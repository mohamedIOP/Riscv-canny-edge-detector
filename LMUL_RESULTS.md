...
|------|-------------------|-------------------|-------------------|
| 128  | 0.067263          | 0.055456          | 0.022150          |
| 256  | 0.040058          | 0.022741          | 0.017305          |
| 512  | 0.025147          | 0.015852          | 0.013653          |

### Relative speedup vs LMUL=1

| VLEN | LMUL=2 vs LMUL=1 | LMUL=4 vs LMUL=1 |
|------|------------------|------------------|
| 128  | ~1.21x faster    | ~3.04x faster    |
| 256  | ~1.76x faster    | ~2.32x faster    |
| 512  | ~1.59x faster    | ~1.84x faster    |
...
from ~3.0x down to ~1.8x.

**LMUL=1 is consistently the slowest configuration.** At VLEN=128, LMUL=1
...
QEMU user-mode timing has some run-to-run variance (it is not
cycle-accurate), so absolute numbers differ slightly between runs, but the
relative ordering (LMUL=4 > LMUL=2 > LMUL=1 at every VLEN) is consistent
and reproducible.
