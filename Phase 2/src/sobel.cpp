        ...
        for (size_t x = 0; x < width; x++) {
            ...
            for (int ky = -radius; ky <= radius; ky++) {
                for (int kx = -radius; kx <= radius; kx++) {
                    ...
                    sum_x += (int32_t)pixel * SOBEL_X[k_idx];
                    sum_y += (int32_t)pixel * SOBEL_Y[k_idx];
                }
            }
            
            gx_out[y * width + x] = (int16_t)sum_x;
            gy_out[y * width + x] = (int16_t)sum_y;
        }
    }
}
